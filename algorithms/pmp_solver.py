...
class PMPSolver:
    ...
    # =================== 7. Exhaustive Search ===================
    def solve_exhaustive(self) -> Tuple[float, Dict, float]:
        start_time = time.perf_counter()
        best_lat: float = float('inf')
        best_plan: Dict = {}

        cut_positions = range(self.L + 1)
        for cuts in itertools.combinations_with_replacement(cut_positions, self.K - 1):
            splits: List[int] = [0] + list(cuts) + [self.L]
            total_lat: float = 0.0
            plan: Dict = {}
            valid: bool = True
        ...
        return best_lat, best_plan, elapsed_ms