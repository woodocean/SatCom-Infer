...
`plot_avg_tho_vs_real.py` 负责从统一长表 `results_long.csv` 读取实验结果并自动画图。

它会根据 `exp_type` 自动选择图形：

- `algo_effectiveness`：画算法对比柱状图。
- `isl_bandwidth_sensitivity`：画 ISL 带宽敏感性折线图。
- `gsl_bandwidth_sensitivity`：画 GSL 带宽敏感性折线图。
...
如果你不传 `--run-id`，脚本会默认读取最新一批 `run_id`。
...
- `algo_effectiveness`：`theory_vs_experiment_analysis.png`
- `isl_bandwidth_sensitivity`：`isl_bandwidth_sensitivity_analysis.png`
- `gsl_bandwidth_sensitivity`：`gsl_bandwidth_sensitivity_analysis.png`
